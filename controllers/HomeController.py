...
def dashboard():
    ...
    if current_user.get_type() == "ADMINISTRATOR":
        ...
        moderation["pending_posts_count"] = Post().get_pending_posts_count()
        moderation["pending_users_count"] = len(User().get_pending_users())

    elif current_user.isModerator():
        group_ids = GroupModerator(user=current_user).getGroupsCanModerate()
        moderation["pending_posts_count"] = Post().get_pending_posts_count(group_ids=group_ids)
    else:
        counts["active_posts_count"] = len(Post().get_active_posts(user_id=current_user.id))
        
    
    return render_template("home/dashboard.html",permissions=permissions, moderation=moderation, counts=counts)
